...
export default function AddressForm({
    ...
    const debouncedGetAddressSuggestions = debounce(async (address: string) => {
        if (address === lastAddressDebounce || !selectedCountry) return;
        setLastAddressDebounce(address);
    ...
    useEffect(() => {
        if (address1 && address1.length >= 2) {
            debouncedGetAddressSuggestions(address1);
        }
    ...
    const handleFormSubmit = async (data: FormValues): Promise<void> => {
        ...
        const customerData = {
            mainAddress: {
                ...
                city: data?.country === "JP" ? "" : data?.city,
                state: data?.country === "JP" ? "" : data?.state,
                zip: data?.zip
            }
        };
        try {
            ...
            console.log("updateMainAddress", updateMainAddress);
                    ...
                    <div css={styles.row(theme)}>
                        ...
                        >
                            <TextField
                                ...
                                onChange={(): void => setHintsOpen(true)}
                            />
